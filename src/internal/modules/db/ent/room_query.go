...
func (rq *RoomQuery) Order(o ...room.Order) *RoomQuery {
	rq.order = append(rq.order, o...)
	return rq
}

// QueryContains chains the current query on the "contains" edge.
func (rq *RoomQuery) QueryContains() *PatientQuery {
...
func (rq *RoomQuery) Clone() *RoomQuery {
	...
	return &RoomQuery{
		config:       rq.config,
		ctx:          rq.ctx.Clone(),
		order:        append([]room.Order{}, rq.order...),
		inters:       append([]Interceptor{}, rq.inters...),
		predicates:   append([]predicate.Room{}, rq.predicates...),
		withContains: rq.withContains.Clone(),
...
// WithContains tells the query-builder to eager-load the nodes that are connected to
// the "contains" edge. The optional arguments are used to configure the query builder of the edge.
func (rq *RoomQuery) WithContains(opts ...func(*PatientQuery)) *RoomQuery {
	query := (&PatientClient{config: rq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	rq.withContains = query
	return rq
}
...
func (rq *RoomQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Room, error) {
	...
	if query := rq.withContains; query != nil {
		if err := rq.loadContains(ctx, query, nodes,
			func(n *Room) { n.Edges.Contains = []*Patient{} },
			func(n *Room, e *Patient) { n.Edges.Contains = append(n.Edges.Contains, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (rq *RoomQuery) loadContains(ctx context.Context, query *PatientQuery, nodes []*Room, init func(*Room), assign func(*Room, *Patient)) error {
	...
	for _, n := range neighbors {
		...
		node, ok := nodeids[fk]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "roomNumber" returned %v for node %v`, fk, n.ID)
		}
		assign(node, n)
	}
	return nil
